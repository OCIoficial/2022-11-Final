...
\begin{document}
  ...
  \large{\phase}
  \vskip 10pt
  \normalsize{\it 26 de Noviembre, \the\year}
...
\item El nombre del archivo debe tener la extensión \texttt{.cpp},
  \texttt{.java} o \texttt{.py} dependiendo de si la solución está escrita en
  \cpp, \java o \python respectivamente.
...
\item Toda lectura debe ser hecha desde la {\bf entrada estándar} usando, por
  ejemplo, las funciones \verb+scanf+ o \verb+std::cin+ en \cpp, la clase
  \verb+BufferedReader+ en \java o la función \texttt{input} en \python.
...
\item Toda escritura debe ser hecha hacia la {\bf salida estándar} usando, por
  ejemplo, las funciones \verb+printf+, \verb+std::cout+ en \cpp, la función
  \verb+System.out.println+ en \java o la función \texttt{print} en \python.
